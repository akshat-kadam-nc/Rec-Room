...
export default async function JukeboxAdminPage({ params }: Context) {
  ...
  return <JukeboxStudio slug={slug} initialDraft={(document?.draft ?? []) as RoomPlaylist[]} initialPublished={(document?.playlists ?? []) as RoomPlaylist[]} initialHasDraft={Array.isArray(document?.draft)} initialPlayerStyle={(document?.draftPlayerStyle ?? document?.playerStyle ?? "rec-room") as "rec-room" | "saloon"} initialAutoplayPlaylistId={(document?.draftAutoplayPlaylistId ?? document?.autoplayPlaylistId ?? null) as string | null} />;
}
