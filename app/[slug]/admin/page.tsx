...
export default async function TenantAdminPage({ params }: Context) {
  ...
  return <><AdminStudio slug={slug} ownerName={configuration?.ownerName || access.session.user.name} initialContent={safeContent} initialConfiguration={safeConfiguration} initialPlaylists={safePlaylists} /><div className="studio-logout"><LogoutButton /></div></>;
}
