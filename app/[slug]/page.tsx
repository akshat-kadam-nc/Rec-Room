...
export default async function TenantRoomPage({ params }: Context) {
  ...
  return <RecRoom slug={slug} ownerName={room.configuration?.ownerName || room.tenant.name} roomTitle={room.configuration?.title || "The Rec Room"} city={room.configuration?.city || room.configuration?.locationLabel?.split("/")[0]?.trim()} timeZone={room.configuration?.timeZone} theme={room.configuration?.background?.theme} templateId={room.configuration?.background?.templateId} markerStyle={room.configuration?.objectVariation?.markers} enabledComponents={room.configuration?.objectVariation?.enabledComponents} volumes={content?.libraryVolumes} collections={content?.roomCollections} playlists={room.playlists as RoomPlaylist[]} playerStyle={room.playerStyle as "rec-room" | "saloon"} />;
}
