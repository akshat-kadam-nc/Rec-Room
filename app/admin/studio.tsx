...
type AppearanceData = { ownerName?: string; title?: string; city?: string; timeZone?: string; locationLabel?: string; background?: { templateId?: string; theme?: string }; objectVariation?: { enabledComponents?: RoomComponent[]; markers?: string } };
...
export function AdminStudio({ slug = "akshat", ownerName = "Akshat Kadam", initialContent, initialConfiguration, initialPlaylists }: { slug?: string; ownerName?: string; initialContent?: StudioContent; initialConfiguration?: StudioConfiguration; initialPlaylists?: PlaylistDocument }) {
  ...
  const chooseSection = (id: string) => { if (id === "jukebox") { window.location.assign(`/${slug}/admin/jukebox`); return; } const targetSection = sections.find((item) => item.id === id) ?? sections[1]; setAppearanceMode(false); setSectionId(id); setEntryIndex(0); loadEditor(targetSection, 0); setNotice(""); };
  ...
  const saveAppearance = async () => {
    ...
    const response = await fetch(`/api/tenants/${slug}/content`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ ownerName: roomOwner, title: roomTitle, city, timeZone, theme, templateId, markerStyle, enabledComponents }) });
