...
export function AdminStudio({ slug = "akshat", ownerName = "Akshat Kadam", initialContent, initialConfiguration, initialPlaylists }: { slug?: string; ownerName?: string; initialContent?: StudioContent; initialConfiguration?: StudioConfiguration; initialPlaylists?: PlaylistDocument }) {
  ...
  const chooseSection = (id: string) => { if (id === "jukebox") { window.location.assign(`/${slug}/admin/jukebox`); return; } const targetSection = sections.find((item) => item.id === id) ?? sections[1]; setNotesMode(false); setAppearanceMode(false); setSectionId(id); setEntryIndex(0); loadEditor(targetSection, 0); setNotice(""); };
  ...
  return <main className="admin-studio">
    ...
    <div className="admin-workspace">
      <aside className="admin-sections"><div><span>ROOM STUDIO</span><button type="button" onClick={() => previewAction("Collection creation is coming next.")}>＋</button></div><nav><button type="button" className={appearanceMode && !notesMode ? "is-active" : ""} onClick={() => { setNotesMode(false); setAppearanceMode(true); setNotice(""); }}><i>00</i><span><small>ROOM</small><strong>Appearance</strong></span><b>✦</b></button>{sections.map((item) => <button type="button" key={item.id} className={!notesMode && !appearanceMode && sectionId === item.id ? "is-active" : ""} onClick={() => chooseSection(item.id)}><i>{item.number}</i><span><small>{item.group}</small><strong>{item.title}</strong></span><b>{item.source === "AUTOMATED" ? "↻" : String(sampleEntries[item.id]?.length ?? 0).padStart(2, "0")}</b></button>)}</nav><button className={`admin-notes-nav ${notesMode ? "is-active" : ""}`} type="button" onClick={() => { setNotesMode(true); setNotice(""); }}><i>10</i><span><small>INBOX</small><strong>Visitor Notes</strong></span><b>✎</b></button><footer><span>DATABASE</span><strong>CONNECTED</strong><small>TENANT: /{slug}</small></footer></aside>
      {notesMode ? <NotesStudio embedded ownerName={ownerName} slug={slug} /> : <>
